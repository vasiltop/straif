...
func _test_valid_server(t: TestCase) -> void:
	...
	if result.is_ok():
		t.check_equal(
				...
				"Server args should set role to DEDICATED_SERVER",
		)
...
func _test_server_missing_or_invalid_positionals(t: TestCase) -> void:
	...
	var unsupported_mode := RuntimeOptions.parse(
			PackedStringArray(["server", "DM-1", "3005", "5", "deathmtach"]),
			false,
	)
...
func _test_unknown_command_flag_and_extra(t: TestCase) -> void:
	...
	t.check(
			not RuntimeOptions.parse(PackedStringArray(["--not-a-real-flag"]), false).is_ok(),
			"Unknown flag should error",
	)
	var extra_server_positional := RuntimeOptions.parse(
			PackedStringArray(["server", "DM-1", "3005", "5", "deathmatch", "extra"]),
			false,
	)
	t.check(not extra_server_positional.is_ok(), "Extra positional after valid server args should error")
...
func _test_valid_connect_with_e2e(t: TestCase) -> void:
	...
	if result.is_ok():
		t.check_equal(
				result.options.role,
				RuntimeOptions.Role.CONNECT_CLIENT,
				"connect should set role to CONNECT_CLIENT",
		)
...
func _test_e2e_value_flags_reject_flag_values(t: TestCase) -> void:
	var instance_flag_as_value := RuntimeOptions.parse(
			PackedStringArray(["connect", "127.0.0.1", "5000", "--e2e", "--e2e-instance", "--e2e-control-port", "6001"]),
			true,
	)
	...
	var instance_missing_value := RuntimeOptions.parse(
			PackedStringArray(["connect", "127.0.0.1", "5000", "--e2e", "--e2e-instance"]),
			true,
	)
...
func _test_e2e_role_gating(t: TestCase) -> void:
	...
	if server_e2e.is_ok():
		t.check_equal(
				server_e2e.options.role,
				RuntimeOptions.Role.DEDICATED_SERVER,
				"server e2e keeps the dedicated server role",
		)
		t.check(server_e2e.options.e2e, "server e2e should set e2e to true")

	var menu_client_e2e := RuntimeOptions.parse(
			PackedStringArray(["--e2e", "--e2e-instance", "alice", "--e2e-control-port", "6001"]),
			true,
	)
	...
	var instance_without_e2e := RuntimeOptions.parse(
			PackedStringArray(["connect", "127.0.0.1", "5000", "--e2e-instance", "alice"]),
			true,
	)
	t.check(not instance_without_e2e.is_ok(), "An e2e-only flag without --e2e should be rejected")

func _test_server_rejects_flag_like_positionals(t: TestCase) -> void:
	var server_name_like_flag := RuntimeOptions.parse(
			PackedStringArray(["server", "--sneaky", "3005", "5", "deathmatch"]),
			false,
	)
	...
	var server_mode_like_flag := RuntimeOptions.parse(
			PackedStringArray(["server", "DM-1", "3005", "5", "--sneaky"]),
			false,
	)
