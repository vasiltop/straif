...
func _test_e2e_connect_selects_readable_fake_identities(t: TestCase) -> void:
	...
	if alice.is_ok() and bob.is_ok():
		t.check(
			alice.context.identity is FakeIdentityProvider, "E2E connect must select the fake identity, never Steam"
		)
		t.check(alice.context.server_registry == null, "E2E connect client needs no server registry")
		t.check_equal(
			alice.context.identity.display_name(), "Alice", "E2E identity name should read directly from the instance"
		)
		t.check_equal(
			bob.context.identity.display_name(), "Bob", "E2E identity name should read directly from the instance"
		)
...
func _test_e2e_server_selects_recording_registry(t: TestCase) -> void:
	var args := PackedStringArray(
		["server", "DM-1", "3005", "8", "deathmatch", "--e2e", "--e2e-instance", "srv", "--e2e-control-port", "6002"]
	)
...
func _test_invalid_role_or_dependencies_fail(t: TestCase) -> void:
	...
	if with_bridge.is_ok():
		t.check(
			with_bridge.context.server_registry is HttpServerRegistry, "Dedicated server must select the HTTP registry"
		)
...
func _test_game_manager_publishes_through_injected_registry(t: TestCase) -> void:
	...
	if snapshots.size() == 1:
		...
		t.check_equal(
			snap["map"], "de_dust", "GameManager must assemble the snapshot itself and strip the .tscn suffix"
		)
