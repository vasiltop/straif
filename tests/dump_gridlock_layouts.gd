...
#
# Run: godot --headless --script res://tests/dump_gridlock_layouts.gd
...
func _run() -> void:
	...
	for seed in SEEDS:
		...
		arr.append({"x": 0.0, "y": 0.0, "z": GridlockGenerator.START_Z, "sx": 10.0, "sz": 20.0, "turn": false, "plaza": false})
		for i in range(COUNT):
			...
			arr.append({
				"x": c.x, "y": c.y, "z": c.z,
				"sx": sz.x, "sz": sz.y,
				"turn": b.is_turn, "plaza": b.is_plaza,
			})
		out[str(seed)] = arr

	var f := FileAccess.open("res://tests/gridlock_layouts.json", FileAccess.WRITE)
	f.store_string(JSON.stringify(out))
	f.close()
	print("wrote gridlock_layouts.json with %d seeds" % SEEDS.size())
	quit(0)
