...
func _run() -> void:
	var aim_trainer_script_file := FileAccess.open("res://src/maps/aim/aim_trainer.gd", FileAccess.READ)
	_check(aim_trainer_script_file != null, "Expected aim_trainer.gd to open")
	if aim_trainer_script_file != null:
		var aim_trainer_source := aim_trainer_script_file.get_as_text()
		_check(aim_trainer_source.contains("int(round(average_reaction_ms))"), "Aim trainer should round average reaction to int before submission")
		_check(not aim_trainer_source.contains("theme_override_constants."), "Aim trainer should avoid invalid theme override dot access")
		_check(not aim_trainer_source.contains("theme_override_font_sizes."), "Aim trainer should avoid invalid theme override font-size dot access")
	...
	if aim_trainer != null:
		...
		_check(aim_trainer.has_node("HUD/ResultsPanel/LeaderboardScroll/LeaderboardRows"), "Aim trainer should expose bounded leaderboard rows")
		_check(aim_trainer.has_method("calculate_accuracy"), "Aim trainer should expose calculate_accuracy")
		_check(is_equal_approx(aim_trainer.calculate_accuracy(0, 0), 0.0), "Accuracy helper should avoid divide-by-zero")
		...
		if leaderboard_scroll != null:
			_check(leaderboard_status.get_rect().end.y <= leaderboard_scroll.get_rect().position.y, "Results status should end before leaderboard rows")
			_check(leaderboard_scroll.get_rect().end.y <= retry_button.get_rect().position.y, "Leaderboard rows should end before result actions")
		...
		_check(spawned_player.has_method("setup"), "Spawned runtime player should expose Player behavior")
		...
		_check(aim_trainer.get_node("HUD/ResultsPanel/LeaderboardScroll/LeaderboardRows").get_child_count() == 1, "Aim trainer should create leaderboard rows at runtime")
		...
		_check(Input.mouse_mode == Input.MOUSE_MODE_VISIBLE, "Finished sessions should keep the mouse visible after pause closes")
