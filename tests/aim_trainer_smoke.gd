...
func _run() -> void:
	...
	if aim_trainer != null:
		...
		_check(aim_trainer.has_node("HUD/ResultsPanel/LeaderboardRows"), "Aim trainer should expose leaderboard rows")
		...
		_check(aim_trainer.get_node("HUD/ResultsPanel/LeaderboardRows").get_child_count() == 1, "Aim trainer should create leaderboard rows at runtime")
