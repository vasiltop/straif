...
app.get(
	...
	describeRoute({
		description:
			"Get all runs for a player with their leaderboard position and total runs per map.",
		tags: ["leaderboard"],
		responses: {
			200: {
...
async function get_player_position(time: number, map: string): Promise<number> {
	const [better_runs] = await db
		.select({ count: sql`COUNT(*)`.mapWith(Number) })
		.from(runs)
		.where(and(eq(runs.map_name, map), lt(runs.time_ms, time)));
