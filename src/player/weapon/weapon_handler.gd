...
const DECAL_PROJECTION_TILT := PI / 2.0
...
func _on_sword_hit(body: Node3D) -> void:
	if body is BodyPart:
		if body.owned_by is Player and body.owned_by.is_me():
			return
...
func toggle_sniper_scope() -> void:
	if not current_weapon:
		return
	if not current_weapon.is_sniper:
		return
	if time_since_last_scope < SNIPER_SCOPE_SPAM_TIME:
		return
...
func can_shoot(ghost_bullet: bool) -> bool:
	if current_weapon == null:
		return false
	if mag_ammo <= 0 and not ghost_bullet and not unlimited_ammo:
		return false
...
func _align_bullet_hole_to_surface(decal: Decal, hit_position: Vector3, surface_normal: Vector3) -> void:
	...
	if not surface_faces_straight_up:
		decal.look_at(hit_position + surface_normal, Vector3.UP)
		decal.transform = decal.transform.rotated_local(Vector3.RIGHT, DECAL_PROJECTION_TILT)
...
func _shoot_bullet(ghost_bullet := false) -> void:
	...
	if result != {}:
		...
		if collider is BodyPart:
			...
			if not ghost_bullet:
				body_part.apply_damage(current_weapon.damage, current_weapon.name)
