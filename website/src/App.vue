...
async function load_runs() {
  let res = await fetch(
    `${BASE_URL}/leaderboard/mode/${current_mode.value}/maps/${current_map.value}/runs?page=${current_page.value - 1}`
  );
...
function modify_page(amount) {
  ...
  current_page.value = Math.min(
    Math.max(current_page.value, 1),
    Math.max(total_count.value, 1)
  );
